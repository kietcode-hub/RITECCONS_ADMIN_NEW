...
This is a monorepo (npm workspaces) implementing **RMC-MS** (Hệ thống Quản trị Sản xuất – Kinh doanh Bê tông Thương phẩm — a ready-mix concrete production/sales management system for a 3-branch company), built from the specification set in `files/`. The system is early-stage: a backbone (`business-rules`, `shared-types`, a NestJS API with JWT auth + 3 fully-wired modules) exists; most feature modules are still placeholders. `apps/web` and `apps/mobile` are not yet scaffolded — see `README.md` "Trạng thái hiện tại" for the current checklist before assuming a module is implemented.

**Every endpoint requires a JWT** (global `APP_GUARD`, see `apps/api/src/modules/auth/`) except routes marked `@Public()`. There is no real user database yet — `AuthService` seeds 8 demo users in-memory (password `password123` for all), matching the PRD personas (Hùng-NVKD, Trang-Kỹ thuật, etc.). Get a token via `POST /auth/login`, inspect it via `GET /auth/me`.
...
3. `apps/api` — a NestJS **modular monolith**. `src/modules/<name>/` maps 1:1 to the M01–M16 module map from the PRD/SRS (`admin`=M01, `crm`=M02, `pricing`=M03, `contract`=M04, `order`=M05, `mixdesign`=M06, `quality`=M07, `planning`=M08, `material`=M09, `dispatch`=M10, `fleet`=M11, `delivery`=M12, `ar`=M13, `reporting`=M14, `integration`=M16 — M15 "Mobile App" is a client, not an API module — plus `auth`, which is cross-cutting and not part of the M01–M16 numbering). Most feature modules are currently empty `@Module({})` placeholders; **`src/modules/order`, `planning`, `dispatch` are the fully-wired reference implementations** — read `order.service.ts` to see the intended pattern: call into `@rmc-ms/business-rules` for every rule check, throw `BadRequestException` with the BRULE code in the message when a rule blocks the operation, keep an in-memory store for now (to be replaced by Prisma once each module needs real persistence). Follow this same pattern when fleshing out another module — don't reimplement rule logic inside the module.

**Cross-cutting rule that applies to every module, not just the 3 wired ones:** every business entity carries a `branchId` and must be scoped to the caller's permitted branches **in the service layer** (`packages/business-rules/src/branchScope.ts` → `assertBranchScope`/`isInBranchScope`), not by filtering on the client or trusting a client-supplied branch filter. This is BRULE-17 and is enforced today in `order`/`planning`/`dispatch` — get the scope via `@CurrentUser()` + `toBranchScope()` (`modules/auth/current-user.decorator.ts`) in the controller and pass it down to the service. `BranchScopeViolationError` is a plain `Error`, not an `HttpException` — it's mapped to HTTP 403 by the global `BranchScopeExceptionFilter` (`common/filters/`), not by the services themselves.
